...
def estimate_batch_size(memory_usage_data, total_memory):
    ...
    model.fit(X, y)
    feasible_batch_sizes = np.arange(1, np.max(X))[model.predict(np.arange(1, np.max(X)).reshape(-1, 1)) < total_memory]
...
def find_optimal_batch_size(model, input_size, min_batch_size=32, max_batch_size=512, num_samples=10):
    ...
    for i in tqdm(range(num_samples), desc="Finding optimal batch size"):
        ...
        try:
            memory_allocated = test_batch_size(model, mid_batch_size, input_size, device)
